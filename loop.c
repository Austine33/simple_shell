...
int hash(info_t *info, char **av)
{
	...
	while (r != -1 && builtin_ret != -2)
	{
		...
		{
			set_info(info, av);
			builtin_ret = find_builtin_func(info);
			if (builtin_ret == -1)
				find_command(info);
		}
		else if (is_interactive_func(info))
			_putchar('\n');
		free_info(info, 0);
	}
	write_hist(info);
	free_info(info, 1);
	if (!is_interactive_func(info) && info->status)
		exit(info->status);
	if (builtin_ret == -2)
	{
		if (info->err_num == -1)
 ...
 */
int find_builtin_func(info_t *info)
{
...
void find_command(info_t *info)
{
	...
	path = pathfinder(info, _getenv(info, "PATH="), info->argv[0]);
	if (path)
	{
		info->path = path;
		f_cmd(info);
	...
	{
		if ((is_interactive_func(info) || _getenv(info, "PATH=")
